...
def test_glossary_class():
    ...
    try:
        ...
        assert (
            ja_ai == "人工知能"
        ), "Japanese translation for 'artificial intelligence' incorrect"
        assert (
            ja_ml == "機械学習"
        ), "Japanese translation for 'machine learning' incorrect"
        assert ja_python == "Python", "Japanese translation for 'Python' incorrect"
        assert (
            ja_python_lower is None
        ), "Case-sensitive term should not match with different case"
        ...
        assert (
            "Artificial Intelligence" in found_terms
        ), "Should find 'Artificial Intelligence' in text"
        assert (
            "Machine Learning" in found_terms
        ), "Should find 'Machine Learning' in text"
        ...
        modified_text = glossary.apply_glossary_to_text(
            original_text, "en", "ja", translated_text
        )
        ...
        assert (
            glossary.get_term_count() > 0
        ), "Glossary should not be empty after importing"
...
def test_glossary_functions():
    ...
    try:
        ...
        # Test applying glossary to translated text
        original_text = (
            "Artificial Intelligence and Machine Learning are important fields."
        )
        translated_text = "人工知能と機械学習は重要な分野です。"
        modified_text = apply_glossary_to_text(
            original_text, "en", "ja", translated_text, glossary_file
        )
...
def test_with_translator():
    ...
    try:
        ...
        assert (
            "人工知能" in translated1["slide1_shape1"]
        ), "Glossary term 'artificial intelligence' should be applied"
        assert (
            "機械学習" in translated1["slide1_shape2"]
        ), "Glossary term 'machine learning' should be applied"
        assert (
            "人工知能" in translated1["slide1_notes"]
        ), "Glossary term 'artificial intelligence' should be applied to notes"
        assert (
            "機械学習" in translated1["slide2_notes"]
        ), "Glossary term 'machine learning' should be applied to notes"
