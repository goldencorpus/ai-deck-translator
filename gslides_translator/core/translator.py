"""
Main translator module for the Google Slides Translator application.
"""
...
def translate_text(
    ...
    if resume_file:
        ...
        print(
            f"Resuming translation: {len(recovery_state['translated_items'])} items already translated, {len(text_dict)} remaining"
        )
    ...
    for batch_index, batch in enumerate(batches):
        ...
        try:
            ...
            progress.set_description(
                f"Translating batch {batch_index+1}/{len(batches)}"
            )
    ...
    while recovery_state.get("failed_batches", []):
        print(
            f"Attempting to recover {len(recovery_state['failed_batches'])} failed batches"
        )
            ...
            if failed_batch_index < len(batches):
                ...
                for i, small_batch in enumerate(smaller_batches):
                    translated_small_batch = translate_batch(
                        small_batch,
                        f"{failed_batch_index}.{i}",
                        slide_metadata,
                        source_language,
                        target_language,
                        api_key,
                        max_retries=config.MAX_RETRIES
                        + 1,  # Extra retry for failed batches
                    )
...
def translate_slides(
    ...
    Args:
        presentation_id: ID of the presentation to translate
        source_language: Source language code (default from config)
