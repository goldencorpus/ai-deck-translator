...
QA_LENGTH_RATIO_THRESHOLD = (
    0.5  # Minimum ratio of translation length to original length
)
QA_MAX_LENGTH_RATIO_THRESHOLD = (
    2.0  # Maximum ratio of translation length to original length
)
...
def select_model_for_content(
    content, quality_level, source_language, target_language, content_length=None
):
    """
    ...
    sample_text = (
        next(iter(content.values())) if isinstance(content, dict) else str(content)
    )
    ...
    has_technical_terms = bool(
        re.search(technical_terms_pattern, sample_text, re.IGNORECASE)
    )
...
def translate_with_gemini(
    ...
    while retry_count <= max_retries:
        try:
            ...
            usage_info["cost"] = estimate_cost(prompt_tokens, completion_tokens, model)
...
def perform_quality_check(
    ...
    translated_bullets = len(
        re.findall(r"^\s*[•\-\*]\s+", translated_text, re.MULTILINE)
    )

    if original_bullets != translated_bullets:
        issues.append(
            f"Bullet point count mismatch: {original_bullets} vs {translated_bullets}"
        )

    original_numbering = len(re.findall(r"^\s*\d+\.\s+", original_text, re.MULTILINE))
    translated_numbering = len(
        re.findall(r"^\s*\d+\.\s+", translated_text, re.MULTILINE)
    )

    if original_numbering != translated_numbering:
        issues.append(
            f"Numbered list count mismatch: {original_numbering} vs {translated_numbering}"
        )
    ...
    if original_paragraphs != translated_paragraphs:
        issues.append(
            f"Paragraph count mismatch: {original_paragraphs} vs {translated_paragraphs}"
        )
    ...
    for term in COMMON_TECHNICAL_TERMS:
        if term in original_text and term not in translated_text:
            ...
            similar_terms = [
                ...
                if difflib.SequenceMatcher(None, term, word).ratio()
...
def translate_batch_worker(args):
    ...
    try:
        ...
        content_to_translate = {
            key: value for key, value in zip(batch_keys, batch_values)
        }
        ...
        logger.info(
            f"Batch {batch_index}: Selected model {selected_model} for quality level {quality_level}"
        )
        ...
        if use_cache:
            ...
            for key, value in content_to_translate.items():
                cached = get_cached_translation(
                    value, source_language, target_language, selected_model
                )
                if cached:
        ...
        if qa_enabled:
            ...
            for key, translated_text in translated_batch.items():
                ...
                is_valid, issues = perform_quality_check(
                    original_text, translated_text, source_language, target_language
                )
                ...
                if not is_valid:
                    ...
                    logger.info(
                        f"Batch {batch_index}: Quality issues detected for item {key}: {issues}"
                    )
                    ...
                    logger.info(
                        f"Batch {batch_index}: Fixed translation for item {key}"
                    )
        ...
        return (
            batch_index,
            {},
            {"prompt_tokens": 0, "completion_tokens": 0, "cost": 0, "model": "error"},
        )
                ...
                try:
                    ...
                    with threading.Lock():
                        ...
                        with cost_tracker_lock:
                            cost_tracker["total_prompt_tokens"] += usage_info[
                                "prompt_tokens"
                            ]
                            cost_tracker["total_completion_tokens"] += usage_info[
                                "completion_tokens"
                            ]
                        ...
                        recovery_system["remaining_batches"] = [
                            (idx, batch)
                            for idx, batch in remaining_batches
...
def translate_presentation(
    ...
    # Translate the text
    logger.info(
        f"Translating from {source_language} to {target_language} with quality level: {quality_level}..."
    )
