...
def validate_translation_ids(text_dict, translated_dict, slide_metadata):
    ...
    if missing_ids:
        ...
        text_to_translation = {text: (id, translation) for id, translation in translated_dict.items() 
                               for orig_id, text in text_dict.items() if text == text_dict.get(orig_id)}
        ...
        for missing_id, text in missing_ids.items():
            if text in text_to_translation:
                ...
                logger.info(f"Fixed missing ID {missing_id} by matching content with {similar_id}")
            ...
            for (old_pattern, new_pattern), transformer in patterns.items():
                if re.match(old_pattern, missing_id):
                    ...
                    if transformed_id in translated_dict:
                        ...
                        logger.info(f"Fixed missing ID {missing_id} -> {transformed_id}")
                        break
