...
def translate_batch(texts: List[str], target_language: str, source_language: str = "en") -> List[str]:
    ...
    """
    logger.info(f"Translating {len(texts)} elements from {source_language} to {target_language} using Claude")
    
    # Get API key from config or environment
    api_key = getattr(config, 'CLAUDE_API_KEY', os.environ.get('CLAUDE_API_KEY'))
    if not api_key:
        raise TranslationError("Claude API key not found. Set CLAUDE_API_KEY in config or environment.")
