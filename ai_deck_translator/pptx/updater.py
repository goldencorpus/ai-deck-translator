...
def update_slides(pptx_file, output_file, translated_texts):
    ...
    try:
        ...
        if "presentation_title" in translated_texts and hasattr(prs.core_properties, 'title'):
            ...
            logger.debug(f"Updated presentation title: {translated_texts['presentation_title']}")
...
def update_xml_elements(original_file, updated_file, translated_texts):
    ...
    try:
        ...
        for slide_file in slide_files:
            ...
            if smartart_ids:
                logger.debug(f"Found {len(smartart_ids)} SmartArt elements to update in slide {slide_number}")
                        ...
                        if target_path:
                            ...
                            if os.path.exists(diagram_path):
                                try:
                                    ...
                                    logger.debug(f"Updated SmartArt diagram for {smartart_id}")
                                except Exception as e:
                                    logger.warning(f"Error updating SmartArt diagram: {e}")
            
            # Update XML text elements
            xml_text_ids = [key for key in translated_texts.keys() if key.startswith(f"slide{slide_number}_xml_")]
            if xml_text_ids:
                logger.debug(f"Found {len(xml_text_ids)} XML text elements to update in slide {slide_number}")
            ...
            if notes_id in translated_texts:
                ...
                if os.path.exists(notes_slide_path):
                    try:
                        ...
                        logger.debug(f"Updated notes slide XML for slide {slide_number}")
                    except Exception as e:
