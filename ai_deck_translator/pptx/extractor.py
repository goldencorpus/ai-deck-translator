            ...
            try:
                with zipfile.ZipFile(pptx_file, 'r') as zip_ref:
                    ...
                    if not slide_meta["notes"]:
                        try:
                            notes_slide_path = f"ppt/notesSlides/notesSlide{slide_number}.xml"
                            if notes_slide_path in [name for name in zip_ref.namelist()]:
                                notes_content = zip_ref.read(notes_slide_path).decode('utf-8')
                                notes_root = ET.fromstring(notes_content)
                                
                                notes_text = []
                                for text_elem in notes_root.findall('.//a:t', namespaces):
                                    if text_elem.text and text_elem.text.strip():
                                ...
                                if notes_text:
                                    ...
                                    text_dict[notes_id] = "\n".join(notes_text)
                                    slide_meta["notes"] = "\n".join(notes_text)
                                    logger.debug(f"Extracted notes from XML for slide {slide_number}")
                        except Exception as e:
                            logger.warning(f"Error extracting notes from XML for slide {slide_number}: {e}")
            except Exception as e:
                logger.warning(f"Error during deep XML extraction for slide {slide_number}: {e}")
