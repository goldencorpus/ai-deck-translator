...
def get_translator_for_model(model: str, api_key: Optional[str] = None, max_retries: int = 3) -> TranslationModel:
    """
    ...
    if model.startswith("claude"):
        return AnthropicTranslator(model, api_key or os.environ.get("CLAUDE_API_KEY"), max_retries)
    elif model.startswith("gpt"):
        return OpenAITranslator(model, api_key or os.environ.get("OPENAI_API_KEY"), max_retries)
    elif model.startswith("gemini"):
        return GeminiTranslator(model, api_key or os.environ.get("GOOGLE_AI_API_KEY"), max_retries)
    else:
