...
from .utils import repair_json, extract_json_blocks, get_model_pricing, estimate_cost, clean_text
        ...
        return {
            ...
            "models": {
                model: {
                    ...
                    "avg_cost_per_call": round(data["cost"] / data["calls"], 4) if data["calls"] > 0 else 0
                }
                for model, data in self.model_costs.items()
            }
        }

def perform_quality_check(original_text: str, translated_text: str, source_language: str, target_language: str) -> Tuple[float, List[str]]:
    """
...
"""
    ...
    try:
        ...
        if "claude" in model.lower():
            ...
            logger.warning(f"Model {model} not supported for fixing issues. Returning original translation.")
            return translated_text
...
def translate_batch(
    ...
    logger.info(f"Batch {batch_index}: Found {cached_items}/{len(batch_items)} items in cache")
    ...
    logger.info(f"Batch {batch_index}: Using {selected_model} for translation with {len(content_to_translate)} items")
    
    # Create content for the model
    content_json = [{"index": item["index"], "text": item["text"], "type": item["type"]} for item in content_to_translate]
    ...
    for attempt in range(max_retries):
        try:
            ...
            logger.error(f"Batch {batch_index}: Translation error (attempt {attempt+1}/{max_retries}): {str(e)}")
    ...
    for translated_item in translated_items:
        ...
        if index is not None and translation and index < len(content_to_translate):
            ...
            for batch_item in batch_items:
                if batch_item['id'] == original_item['id'] and batch_item['text'] == original_item['text']:
    ...
    if qa_enabled:
        ...
        for batch_item in batch_items:
            ...
            if quality_score < 70 or any(issue.startswith("Missing placeholder") for issue in issues):
        ...
        for issue_data in issues_requiring_fixes:
            ...
            logger.info(f"Fixing translation issues for item {batch_item['id']} (score: {issue_data['score']:.1f})")
...
def translate_text(
    ...
    for i, text in enumerate(texts):
        ...
        batches.append((batch, i, {}, source_language, target_language, quality_level, use_cache, qa_enabled, api_key, max_retries, cost_tracker))
    ...
    logger.info(f"Translation completed - Total cost: ${cost_summary['total_cost']:.4f}, Calls: {cost_summary['call_count']}")
...
def translate_pptx(
    ...
    for item in slide_texts:
        ...
        if len(current_batch) >= batch_size or current_batch_char_count + item_char_count > 2000:
            if current_batch:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--quality", default=QUALITY_STANDARD, 
                        choices=[QUALITY_PROFESSIONAL, QUALITY_STANDARD, QUALITY_DRAFT, QUALITY_ECONOMY],
    ...
    parser.add_argument("--batch-size", type=int, default=5, help="Number of text elements per batch (default: 5)")
    ...
    parser.add_argument("--list-recovery", action="store_true", help="List available recovery files and exit")
    ...
    if args.list_recovery:
        ...
        if not recovery_files:
            ...
            for i, file_info in enumerate(recovery_files):
                ...
                print(f"   Languages: {file_info['source_language']} → {file_info['target_language']}")
    ...
    try:
        ...
        print(f"Translation completed successfully in {result['execution_time']:.2f} seconds.")
