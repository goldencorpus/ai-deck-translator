...
class ProgressTracker:
    ...
    def __init__(
        self, total: int, callback: Optional[Callable[[int, int], None]] = None
    ):
        """
    ...
    def update(self, n: int = 1):
        ...
        logger.debug(
            f"Progress updated: {self.current}/{self.total} ({int(100 * self.current / self.total) if self.total > 0 else 0}%)"
        )
...
class CustomTqdm:
    ...
    def update(self, n: int = 1):
        ...
        logger.debug(
            f"Progress updated: {self.n}/{self.total} ({int(100 * self.n / self.total) if self.total > 0 else 0}%)"
        )
...
class WebUITqdm(CustomTqdm):
    ...
    def __init__(
        self, total: int, web_state: Optional[Dict[str, Any]] = None, **kwargs
    ):
        """
...
def create_progress_bar(
    total: int, desc: str = "", web_state: Optional[Dict[str, Any]] = None, **kwargs
) -> CustomTqdm:
    """
