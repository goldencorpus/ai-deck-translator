...
def setup_recovery_system(file_id: str, text_dict: Dict[str, str], 
                         slide_metadata: List[Dict[str, Any]], 
                         source_language: str, target_language: str, 
                         resume_file: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    if resume_file and os.path.exists(resume_file):
        try:
            ...
            if recovery_state["source_language"] != source_language or recovery_state["target_language"] != target_language:
                logger.warning(f"Language mismatch in recovery file: expected {source_language}->{target_language}, "
            ...
            recovery_system["slide_metadata"] = recovery_state.get("slide_metadata", slide_metadata)
            recovery_system["translated_texts"] = recovery_state.get("translated_texts", {})
            recovery_system["remaining_batches"] = recovery_state.get("remaining_batches", [])
            
            recovery_file_path = resume_file
            logger.info(f"Successfully loaded recovery state: {len(recovery_system['translated_texts'])} "
        ...
        recovery_file_path = os.path.join(recovery_dir, f"recovery_{file_id}_{timestamp}.json")
    ...
    def save_recovery_state():
        ...
        try:
            ...
            recovery_state["translated_items"] = len(recovery_system["translated_texts"])
            ...
            logger.debug(f"Saved recovery state to {recovery_file_path}: "
