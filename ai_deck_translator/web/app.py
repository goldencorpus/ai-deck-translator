...
def translate_presentation(input_file, output_file, target_language, service, api_key, translate_notes, session_id):
    """
    ...
    try:
        ...
        translation_state[session_id]["total"] = 1  # Set initial total to at least 1 to avoid division by zero
        ...
        logger.info(f"[{session_id}] Translation state initialized: {translation_state[session_id]}")
        ...
        logger.info(f"[{session_id}] Input type: {'Google Slides' if is_google_slides else 'PowerPoint'}")
        ...
        try:
            ...
            translation_state[session_id]["error"] = "Access denied (403): You don't have permission to access this presentation"
            return
            ...
            if "403" in str(e) or "forbidden" in str(e).lower() or "permission" in str(e).lower():
                ...
                translation_state[session_id]["status"] = "failed" 
                translation_state[session_id]["error"] = "Access denied (403): Make sure the presentation is shared or public"
                return
            else:
                raise  # Re-raise if it's not a permission error
        
        logger.info(f"[{session_id}] Extracted {len(text_elements)} text elements from {len(slide_metadata)} slides")
        ...
        if service == "google":
            ...
            if not api_key:
                logger.error(f"[{session_id}] API key is required for Anthropic service")
        ...
        def progress_callback(current, total):
            ...
            try:
                ...
                if session_id not in translation_state:
                    logger.warning(f"[{session_id}] Progress callback called but session not found in translation state")
                    return
                ...
                if total > 0:
                    ...
                    if current == total and state["status"] != "completed":
                        logger.info(f"[{session_id}] Translation progress reached 100%: {current}/{total}")
                        state["status"] = "completed"
                        logger.info(f"[{session_id}] Translation status updated to 'completed'")
                        
                        # Ensure output_file is set for completed translations
                        if 'output_file' not in state and 'output_file' in translation_state.get(session_id, {}):
                            state['output_file'] = translation_state[session_id]['output_file']
                            logger.info(f"[{session_id}] Set output_file path: {state['output_file']}")
                else:
                    state["percent"] = 0
                    logger.warning(f"[{session_id}] Total is 0, setting progress percentage to 0")
        ...
        if total_elements > 0:
            translation_state[session_id]["percent"] = 0
        logger.info(f"[{session_id}] Initial progress state: {translation_state[session_id]}")
        ...
        logger.info(f"[{session_id}] Translated {len(translated_elements)} text elements")
        ...
        try:
            ...
            translation_state[session_id]["status"] = "failed"
            translation_state[session_id]["error"] = "Access denied (403): You don't have permission to update this presentation"
            return
        except Exception as e:
            if "403" in str(e) or "forbidden" in str(e).lower() or "permission" in str(e).lower():
                logger.error(f"[{session_id}] Permission error (403): {e}")
                translation_state[session_id]["status"] = "failed" 
                translation_state[session_id]["error"] = "Access denied (403): Make sure the presentation is shared with edit permissions"
                return
            else:
                raise  # Re-raise if it's not a permission error
        
        if success:
            logger.info(f"[{session_id}] Successfully updated presentation: {output_file}")
            
            # Verify that the output file actually exists on disk
            if not os.path.exists(output_file):
                logger.error(f"[{session_id}] Output file expected but not found at: {output_file}")
                translation_state[session_id]["status"] = "failed"
            ...
            logger.info(f"[{session_id}] Translation completed. Final state: {translation_state[session_id]}")
        else:
            logger.error(f"[{session_id}] Failed to update presentation")
            translation_state[session_id]["status"] = "failed"
            translation_state[session_id]["error"] = "Failed to update presentation"
            logger.error(f"[{session_id}] Translation failed. Final state: {translation_state[session_id]}")
    ...
    logger.info(f"[{session_id}] Translation thread completed. State: {translation_state[session_id]}")
...
def create_app(debug=False):
    ...
    app.config["UPLOAD_FOLDER"] = os.path.join(tempfile.gettempdir(), "ai_deck_translator_uploads")
            ...
            if 'last_updated' in state and current_time - state['last_updated'] > 3600:
                ...
                if 'output_file' in state and os.path.exists(state['output_file']):
                    try:
                        ...
                        logger.info(f"Removed expired output file for session {session_id}")
                    except Exception as e:
                        logger.error(f"Error removing output file for session {session_id}: {e}")
        
        # Remove expired sessions
        for session_id in expired_sessions:
            del translation_state[session_id]
            logger.info(f"Cleaned up expired translation state for session {session_id}")
    ...
    def translate():
        ...
        if slides_id:
            ...
            translation_state[session_id]["note"] = "Working with Google Slides requires proper sharing permissions. The presentation must be shared with at least view access to extract text, and edit access to update it."
            ...
            thread = threading.Thread(
                ...
                args=(slides_id, output_file, target_language, service, api_key, translate_notes, session_id)
            )
            thread.daemon = True
            thread.start()
            
            logger.info(f"[{session_id}] Started translation thread for Google Slides with ID: {slides_id}")
        ...
        if file and allowed_file(file.filename, ALLOWED_EXTENSIONS):
            ...
            thread = threading.Thread(
                target=translate_presentation,
                args=(input_file, output_file, target_language, service, api_key, translate_notes, session_id)
            )
            thread.daemon = True
            thread.start()
    ...
    def progress():
        ...
        if session_id not in translation_state:
            logger.warning(f"Progress page requested but translation state not found for session {session_id}")
            translation_state[session_id] = {
                "status": "idle",
                "progress": 0,
                "total": 0,
        ...
        for field in required_fields:
            if field not in translation_state[session_id]:
                logger.warning(f"Field {field} missing in translation state for session {session_id}, initializing it")
                if field == "status":
                    translation_state[session_id][field] = "idle"
                else:
                    translation_state[session_id][field] = 0
        
        logger.info(f"Rendering progress page with state: {translation_state[session_id]}")
    ...
    def status():
        ...
        if "session_id" not in session:
            ...
            # Check if translation state exists
            if session_id not in translation_state:
                logger.warning(f"Session {session_id} exists but no translation state found")
                response = make_response(jsonify(default_status))
            else:
                ...
                if "status" not in state_copy or not isinstance(state_copy["status"], str):
                    logger.warning(f"Invalid status in state for session {session_id}, fixing")
                    state_copy["status"] = "idle" if "status" not in state_copy else str(state_copy["status"])
                
                for field in ["progress", "total", "percent"]:
                    if field not in state_copy or not isinstance(state_copy[field], (int, float)):
                        logger.warning(f"Invalid {field} in state for session {session_id}, fixing")
                        state_copy[field] = 0
    ...
    def download():
        ...
        # Check if translation state exists
        if session_id not in translation_state:
            logger.warning(f"[{session_id}] Download requested but translation state not found")
            flash("No translation state found for your session", "error")
            return redirect(url_for("index"))
        
        # Log the entire state for debugging
        logger.info(f"[{session_id}] Current translation state: {translation_state[session_id]}")
        
        # Check if output file exists in the state
        if "output_file" not in translation_state[session_id]:
            logger.warning(f"[{session_id}] Download requested but no output_file available in state")
            ...
            if translation_state[session_id].get("status") == "completed":
                ...
                try:
                    ...
                    matching_files = [f for f in os.listdir(temp_dir) if session_id in f and f.endswith(".pptx")]
                    
                    if matching_files:
                        # Find the most recently modified file
                        latest_file = max(matching_files, key=lambda f: os.path.getmtime(os.path.join(temp_dir, f)))
                        recovered_path = os.path.join(temp_dir, latest_file)
                        
                        if os.path.exists(recovered_path):
                            logger.info(f"[{session_id}] Recovered output file: {recovered_path}")
                            translation_state[session_id]["output_file"] = recovered_path
                        else:
                            logger.warning(f"[{session_id}] Recovery attempt failed - file does not exist: {recovered_path}")
                    else:
                        logger.warning(f"[{session_id}] Recovery attempt failed - no matching files found")
                except Exception as e:
                    logger.error(f"[{session_id}] Error during recovery attempt: {e}")
            
            # Check again if output_file exists after recovery attempt
            if "output_file" not in translation_state[session_id]:
                flash("No translated file available. The translation process may not have completed successfully.", "error")
                return redirect(url_for("index"))
        
        # If progress is 100% but status is not completed, fix it
        if (translation_state[session_id].get("progress", 0) == translation_state[session_id].get("total", 0) and 
            translation_state[session_id].get("total", 0) > 0 and
            translation_state[session_id].get("status") != "completed"):
            logger.info(f"[{session_id}] Progress is 100% but status is not completed, fixing it")
            translation_state[session_id]["status"] = "completed"
        ...
        if not os.path.exists(output_file):
            ...
            try:
                temp_dir = app.config["UPLOAD_FOLDER"]
                all_pptx_files = [f for f in os.listdir(temp_dir) if f.endswith(".pptx")]
                # Sort by modification time, newest first
                all_pptx_files.sort(key=lambda f: os.path.getmtime(os.path.join(temp_dir, f)), reverse=True)
                
                if all_pptx_files:
                    # Take the most recently modified PPTX file
                    newest_file = os.path.join(temp_dir, all_pptx_files[0])
                    logger.info(f"[{session_id}] Attempting to use most recent PPTX file: {newest_file}")
                    ...
                    if os.path.exists(newest_file):
                        ...
                        logger.info(f"[{session_id}] Using alternative file for download: {output_file}")
                    else:
                ...
                flash("The translated file could not be found on the server. The file may have been deleted or failed to generate.", "error")
        ...
        try:
            ...
            # Return the file
            logger.info(f"[{session_id}] Sending file for download (final): {output_file}")
            ...
            flash("Failed to download the file. Please try again or contact support if the issue persists.", "error")
            return redirect(url_for("index"))
