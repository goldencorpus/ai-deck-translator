...
def extract_from_smartart(pptx_file, rels_path, rel_id):
    ...
    with zipfile.ZipFile(pptx_file, "r") as zip_ref:
        ...
        try:
            ...
            # Text in data model
            for text_elem in diagram_root.findall(
                ".//dgm:t",
            ...
            # Text in properties
            for prop in diagram_root.findall(
                ".//dgm:p",
                {"dgm": "http://schemas.openxmlformats.org/drawingml/2006/diagram"},
            ):
                val = prop.get("val")
                        ...
                        for rel in dm_rels.findall(".//Relationship"):
                            if rel.get("Id") == data_model_id:
                                ...
                                if data_model_target.startswith("../"):
                                    ...
                                    data_model_target = os.path.join(
                                        "ppt/diagrams", data_model_target
                                    )
...
def extract_text(pptx_file):
    ...
    for index, slide in enumerate(prs.slides):
        slide_info = {"slide_number": index + 1, "title": "", "content": []}
    ...
    with zipfile.ZipFile(pptx_file, "r") as zip_ref:
        # Process each slide
        slide_xmls = [
            f for f in zip_ref.namelist() if re.match(r"ppt/slides/slide[0-9]+\.xml", f)
        ]
        ...
        for slide_xml in slide_xmls:
                    ...
                    for dgm in graphic.findall(".//a:dgm", namespaces):
                        if (
                            ...
                            smartart_text = extract_from_smartart(
                                pptx_file, slide_rels, rel_id
                            )
                            if smartart_text:
                                object_id = (
                                    f"slide_{slide_num}_smartart_{len(text_dict)}"
                                )
            ...
            for elem in slide_root.findall(".//*"):
                if (
                    elem.text
                    and elem.text.strip()
                    and not any(elem.text.strip() in v for v in text_dict.values())
                ):
            ...
            for elem in slide_root.findall(".//p:nvPr", namespaces):
                alt_text_elem = elem.find(".//a:altTxt", namespaces) or elem.find(
                    ".//p:extLst//p:ext//a14:alt", namespaces
                )
                if (
                    alt_text_elem is not None
                    and alt_text_elem.text
                    and alt_text_elem.text.strip()
                ):
                    if not any(
                        alt_text_elem.text.strip() in v for v in text_dict.values()
                    ):
    ...
    print(
        f"  - Special elements extraction: {extraction_stats['special_elements']} elements"
    )
...
def setup_recovery_system(
    ...
    if resume_file and os.path.exists(resume_file):
        ...
        recovery_file = os.path.join(
            recovery_dir, f"recovery_ultimate_{file_id}_{timestamp}.json"
        )
        recovery_state = {
            "file_id": file_id,
            "completed_batches": [],
...
def translate_text(
    ...
    if missing_keys:
        ...
        if len(missing_keys) > 0:
            ...
            final_chunk_count = (
                len(missing_items) + final_chunk_size - 1
            ) // final_chunk_size  # Ceiling division

            if final_chunk_count > 1:
                print(
                    f"Processing final {len(missing_items)} items in {final_chunk_count} smaller chunks"
                )
                ...
                for chunk_idx in range(final_chunk_count):
                    ...
                    try:
                        print(
                            f"Processing final chunk {chunk_idx+1}/{final_chunk_count} with {len(chunk_dict)} items"
                        )

                        # Use the translate_batch function for consistent handling
                        chunk_result = translate_batch(
                            chunk_dict,
                            f"final_{chunk_idx+1}",
                            slide_metadata,
                            source_language,
                            target_language,
                            api_key=api_key,
