def implement_batch_recovery(
    ...
    def retry_failed_batches():
        ...
        for failed_batch_info in list(recovery_state["failed_batches"]):
            ...
            print(
                f"Retrying batch {batch_index} with smaller size: {original_size} → {new_batch_size}"
            )
            ...
            sub_batches = [
                dict(retry_items[i : i + new_batch_size])
            ...
            for i, sub_batch in enumerate(sub_batches):
                print(
                    f"Processing sub-batch {i+1}/{len(sub_batches)} for failed batch {batch_index}"
                )
                ...
                success, result = process_batch(
                    sub_batch, f"{batch_index}.{i+1}", new_batch_size
                )
            ...
            if sub_success_count == len(sub_batches):
                ...
                print(
                    f"Partial recovery of batch {batch_index}: {sub_success_count}/{len(sub_batches)} sub-batches"
                )
...
def recover_translation():
    ...
    parser.add_argument(
        "--recovery-file", required=True, help="Path to the recovery JSON file"
    )
    parser.add_argument(
        "--retry-failed",
        action="store_true",
        help="Retry failed batches with smaller chunks",
    )
    parser.add_argument(
        "--list", action="store_true", help="List all recovery files and their status"
    )
    ...
    print(
        f"Translation progress: {translated_items}/{total_items} items ({translated_items/total_items*100:.1f}%)"
    )
    ...
    if translated_items > 0:
        ...
        print(
            f"  python slides_translator.py --extract-translations {args.recovery_file}"
        )
