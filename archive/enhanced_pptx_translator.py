...
def extract_text(pptx_file):
    ...
    def process_text_frame(text_frame, element_id, slide_info):
        ...
        if text_frame.text.strip():
            ...
            if element_id.endswith("_shape_0") or element_id.endswith("_shape_1"):
                if (
                    not slide_info["title"] and len(text_frame.text) < 100
    ...
    def process_shape(shape, shape_id, slide_idx, slide_info):
        ...
        try:
            if (
                hasattr(shape, "shape_properties")
                and hasattr(shape.shape_properties, "title")
                and shape.shape_properties.title
            ):
        ...
        try:
            if hasattr(shape, "chart") and shape.chart:
                ...
                if (
                    ...
                    and shape.chart.chart_title.text_frame
                ):
                    chart_title_id = f"{base_id}_chart_title"
                    process_text_frame(
                        shape.chart.chart_title.text_frame, chart_title_id, slide_info
                    )
                            ...
                            if hasattr(plot, "categories") and plot.categories:
                                for cat_idx, cat in enumerate(plot.categories):
                                    if cat and str(cat).strip():
                                        cat_id = (
                                            f"{base_id}_chart_cat_{plot_idx}_{cat_idx}"
                                        )
        ...
        try:
            if (
                ...
                and not hasattr(shape, "text_frame")
            ):
    ...
    for slide_idx, slide in enumerate(prs.slides):
        slide_info = {"slide_number": slide_idx + 1, "title": "", "content": []}
        ...
        try:
            if hasattr(slide, "notes_slide") and slide.notes_slide:
                for note_shape in slide.notes_slide.shapes:
                    if (
                        hasattr(note_shape, "text_frame")
                        and note_shape.text_frame
                        and note_shape.text.strip()
                    ):
                    ...
                    for shape in slide.shapes:
                        try:
                            if (
                                ...
                                and shape.placeholder_format.idx == placeholder_index
                                and shape.text.strip()
                            ):
    ...
    try:
        for master_idx, master in enumerate(prs.slide_masters):
            master_info = {"master_number": master_idx + 1, "content": []}
    ...
    print(
        f"Enhanced extraction found {len(text_dict)} text elements across {extraction_stats['total_slides']} slides"
    )
    print(
        f"Processed {extraction_stats['shapes_processed']} shapes with {extraction_stats['paragraphs_found']} paragraphs"
    )
...
def update_slides(pptx_file, output_file, translated_texts):
    ...
    def update_text_frame(text_frame, new_text):
        ...
        for para in text_frame.paragraphs:
            ...
            for run in para.runs:
                ...
                if hasattr(run, "font") and run.font:
                    ...
                    run_format["italic"] = (
                        font.italic if hasattr(font, "italic") else None
                    )
                    run_format["underline"] = (
                        font.underline if hasattr(font, "underline") else None
                    )
        ...
        if formatting and formatting[0] and formatting[0]["runs"] and p.runs:
            ...
            if hasattr(run, "font"):
                ...
                if original_format.get("hyperlink") is not None and hasattr(
                    run, "hyperlink"
                ):
    ...
    def process_shape(shape, shape_id, slide_idx):
        ...
        alt_id = f"{base_id}_alt"
        if (
            alt_id in translated_texts
            and hasattr(shape, "shape_properties")
            and hasattr(shape.shape_properties, "title")
        ):
            try:
        ...
        try:
            if hasattr(shape, "chart") and shape.chart:
                ...
                chart_title_id = f"{base_id}_chart_title"
                if (
                    chart_title_id in translated_texts
                    and hasattr(shape.chart, "chart_title")
                    and hasattr(shape.chart.chart_title, "text_frame")
                ):
                    update_text_frame(
                        shape.chart.chart_title.text_frame,
                        translated_texts[chart_title_id],
                    )
                    updated = True
        except:
            pass
        ...
        for slide_idx, slide in enumerate(prs.slides):
            ...
            try:
                if hasattr(slide, "notes_slide") and slide.notes_slide:
                    for note_shape in slide.notes_slide.shapes:
                        note_id = f"slide_{slide_idx+1}_notes"
                        if note_id in translated_texts and hasattr(
                            note_shape, "text_frame"
                        ):
                            update_text_frame(
                                note_shape.text_frame, translated_texts[note_id]
                            )
            except:
                pass
            ...
            try:
                for elem in ["header", "footer", "date"]:
                    ...
                    if elem_id in translated_texts:
                        placeholder_index = {"header": 2, "footer": 4, "date": 3}.get(
                            elem
                        )
                        if placeholder_index:
                            for shape in slide.shapes:
                                try:
                                    if (
                                        ...
                                        and shape.is_placeholder
                                        and hasattr(shape, "placeholder_format")
                                        and shape.placeholder_format.idx
                                        == placeholder_index
                                        and hasattr(shape, "text_frame")
                                    ):
                                        update_text_frame(
                                            shape.text_frame, translated_texts[elem_id]
                                        )
                                except:
                                    pass
            except:
...
def split_dict_into_smart_batches(
    input_dict, max_input_tokens=150000, prompt_tokens=2000
):
    """
    ...
    def estimate_tokens(text):
        ...
        cjk_chars = sum(
            1 for c in text_str if ord(c) > 0x4E00 and ord(c) < 0x9FFF
    ...
    print(
        f"Batch sizes: min={min(batch_sizes) if batches else 0}, max={max(batch_sizes) if batches else 0}, avg={avg_batch_size:.1f}"
    )
    print(
        f"Estimated token usage efficiency: {(sum(batch_sizes)/total_items)*100:.1f}%"
    )
...
def repair_json(json_content):
    ...
    try:
        ...
        json_content = re.sub(
            r'(?<!\\)\\(?!["\\/bfnrt]|u[0-9a-fA-F]{4})', r"\\\\", json_content
        )
...
def extract_json_blocks(text):
    ...
    try:
        ...
        text = re.sub(
            r"[^}]*$", "", text
        )  # Remove anything after the last }$', '', text)  # Remove anything after the last }
...
def setup_recovery_system(
    ...
    if resume_file and os.path.exists(resume_file):
        ...
        recovery_file = os.path.join(
            recovery_dir, f"recovery_pptx_{file_id}_{timestamp}.json"
        )
        recovery_state = {
            "file_id": file_id,
            "completed_batches": [],
...
def translate_batch(
    ...
    def estimate_cost(prompt_tokens, completion_tokens, model="claude-3-7-sonnet"):
        ...
        model_rates = rates.get(
            model, rates["claude-3-7-sonnet"]
        )  # Default to sonnet if not found
    ...
    if not api_key:
        raise ValueError(
            "Claude API key must be provided either as an argument or via CLAUDE_API_KEY environment variable"
        )

    client = anthropic.Anthropic(
        api_key=api_key, default_headers={"anthropic-beta": "output-128k-2025-02-19"}
    )
    ...
    for retry in range(max_retries + 1):
        try:
            ...
            batch_cost = estimate_cost(
                prompt_tokens, completion_tokens, "claude-3-7-sonnet"
            )
            ...
            print(
                f"Batch {batch_index} token usage: {prompt_tokens} input + {completion_tokens} output tokens"
            )
            print(
                f"Batch {batch_index} cost: ${batch_cost['total_cost']:.4f} (${batch_cost['input_cost']:.4f} input + ${batch_cost['output_cost']:.4f} output)"
            )

            # Show running total cost
            if cost_tracker is not None:
                print(
                    f"Running total: ${cost_tracker['total_cost']:.4f} for {cost_tracker['api_calls']} API calls"
                )
            ...
            if "```json" in translated_text:
                json_content = (
                    translated_text.split("```json")[1].split("```")[0].strip()
                )
            ...
            try:
                ...
                try:
                    ...
                    if extracted_result:
                        ...
                        print(
                            f"Extracted {len(batch_result)} items through JSON block extraction"
                        )
                    else:
                        if retry < max_retries:
                            print(
                                f"JSON parsing failed on attempt {retry+1}, retrying..."
                            )
                            time.sleep(3)
                            continue
                        else:
            ...
            if retry < max_retries:
                ...
                print(
                    f"All {max_retries + 1} attempts failed for batch {batch_index}: {e}"
                )
                raise e
...
def translate_text(
    ...
    def deduplicate_content(input_dict):
        ...
        print(
            f"Reduced from {len(input_dict)} to {len(unique_content)} unique content items to translate"
        )
    ...
    if not recovery_state["translated_items"]:
        ...
        unique_text_dict = {
            ...
            if k
            not in [rep_key for rep_key in recovery_state["duplicates_map"].values()]
        }
        duplicates_map = recovery_state["duplicates_map"]
        print(
            f"Resumed with {len(recovery_state['translated_items'])} already translated items"
        )

    remaining_dict = {
        k: v
        for k, v in unique_text_dict.items()
    ...
    if not remaining_dict:
        ...
        print(
            f"Using smaller batch size for {target_language} translation"
            if target_language in ["ja", "zh", "ko"]
            else "Using standard batch size"
        )
        batches = split_dict_into_smart_batches(
            remaining_dict, max_input_tokens=max_tokens, prompt_tokens=prompt_tokens
        )
        ...
        if recovery_state["failed_batches"]:
            ...
            for failed_batch in list(recovery_state["failed_batches"]):
                ...
                sub_batches = [
                    dict(retry_items[i : i + chunk_size])
                    for i in range(0, len(retry_items), chunk_size)
                ]

                print(
                    f"Split failed batch {batch_id} into {len(sub_batches)} smaller chunks of size ~{chunk_size}"
                )
                ...
                for i, sub_batch in enumerate(sub_batches):
                    ...
                    try:
                        print(
                            f"Processing sub-batch {i+1}/{len(sub_batches)} for failed batch {batch_id}"
                        )
                        ...
                        print(
                            f"Error in sub-batch {i+1} of failed batch {batch_id}: {e}"
                        )
                        continue
    ...
    print(
        f"Reconstructed full translation dictionary with {len(full_translated_dict)} items"
    )

    missing_keys = set(text_dict.keys()) - set(full_translated_dict.keys())
    if missing_keys:
        print(
            f"Warning: {len(missing_keys)} keys were not translated: {list(missing_keys)[:5]}..."
        )
        if len(missing_keys) > 0:
            print(
                f"Attempting to translate {len(missing_keys)} missing keys in a final batch..."
            )
            missing_dict = {k: text_dict[k] for k in missing_keys if k in text_dict}

            try:
                structured_context = json.dumps(
                    slide_metadata, ensure_ascii=False, indent=2
                )
...
"""
                ...
                def estimate_cost(
                    prompt_tokens, completion_tokens, model="claude-3-7-sonnet"
                ):
                    # Claude 3.5 Sonnet pricing: $3 per 1M input tokens, $15 per 1M output tokens
                    # Claude 3 Sonnet pricing: $3 per 1M input tokens, $15 per 1M output tokens
                    rates = {
                ...
                print(
                    f"Final batch token usage: {prompt_tokens} input + {completion_tokens} output tokens"
                )
                print(
                    f"Final batch cost: ${batch_cost['total_cost']:.4f} (${batch_cost['input_cost']:.4f} input + ${batch_cost['output_cost']:.4f} output)"
                )
                ...
                print(
                    f"Successfully processed final batch with {len(final_batch)} additional items"
                )

                missing_keys = set(text_dict.keys()) - set(full_translated_dict.keys())
                if missing_keys:
                    print(
                        f"Final warning: {len(missing_keys)} keys still not translated: {list(missing_keys)[:5]}..."
                    )
                else:
    ...
    print(
        f"Total tokens: {cost_tracker['total_input_tokens'] + cost_tracker['total_output_tokens']:,}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--list-recovery", action="store_true", help="List available recovery files"
    )
    ...
    parser.add_argument(
        "--api-key",
        help="Claude API Key (can also be set as CLAUDE_API_KEY environment variable)",
    )
