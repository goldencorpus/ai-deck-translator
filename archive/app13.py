...
def update_slides(
    ...
    for i in range(0, len(requests), batch_size):
        ...
        try:
            ...
            print(
                f"Problem might be in these requests: {json.dumps(problem_batch, indent=2)}"
            )
...
def setup_recovery_system(
    ...
    if resume_file and os.path.exists(resume_file):
        ...
        recovery_file = os.path.join(
            recovery_dir, f"recovery_{presentation_id}_{timestamp}.json"
        )
        recovery_state = {
...
def translate_batch(
    batch, batch_index, slide_metadata, source_language, target_language, max_retries=2
):
    """
    ...
    for retry in range(max_retries + 1):
        try:
            response = client.messages.create(
                ...
                metadata={"user_id": "anonymous_user", "privacy": "strict"},
            )
...
def translate_text(
    text_dict, slide_metadata, source_language, target_language, resume_file=None
):
    client = anthropic.Anthropic(
        api_key=os.getenv("CLAUDE_API_KEY"),
        default_headers={"anthropic-beta": "output-128k-2025-02-19"},
    ...
    if not remaining_dict:
        ...
        batches = split_dict_into_smart_batches(
            remaining_dict, max_input_tokens=150000, prompt_tokens=2000
        )
...
if __name__ == "__main__":
    ...
    translated_texts = translate_text(
        extracted_text, slide_metadata, source_language, target_language, args.resume
    )
